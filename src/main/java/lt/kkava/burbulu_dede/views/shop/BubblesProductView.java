...
@Component
@Scope("prototype")
@Route(value = "product_bubbles_details", layout = MainLayout.class)
@PageTitle("BURBULŲ PARDUOTUVĖ")

public class BubblesProductView extends VerticalLayout implements HasUrlParameter<String> {
    private Product product;
    private final ProductService productService;
    private final BasketService basketService;

    public BubblesProductView(ProductService productService, BasketService basketService) {
    ...
    protected void onAttach(AttachEvent attachEvent) {
        ...
        removeAll();
        if (!isMobile())
            add(new ProductViewComponent(product, basketService, RouteConfiguration.forSessionScope().getUrl(BubblesCollectionView.class)));
        else {
            add(new ProductViewComponentPhone(product, basketService, RouteConfiguration.forSessionScope().getUrl(BubblesCollectionView.class)));
